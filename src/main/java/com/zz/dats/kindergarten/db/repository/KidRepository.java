...
public interface KidRepository extends JpaRepository<KidEntity, Integer> {
    @Query(
        ...
        //for some reason it doesn't work   --> "FROM KidEntity AS k LEFT JOIN k.kindergartenKidsById AS kk LEFT JOIN k.queuesById as q WHERE q.kidByKidId.id NOT IN (SELECT kidByKidId.id FROM QueueEntity WHERE q.kindergartenByKindergartenId.id = ?1 ) AND kk.id IS NULL"
    )
